...
// ⚠️ webview 내부 뷰포트 고착 해제 — 숨김/초기 레이아웃 때 잡힌 작은 게스트 뷰포트가 유지되어
//   페이지가 위쪽 일부만 그려지는 Electron webview quirk. 표시/로드/리사이즈 때마다 강제로 재레이아웃.
function nudgeWv() {
  wv.style.height = '99.99%';
  requestAnimationFrame(() => { wv.style.height = '100%'; });
}
wv.addEventListener('dom-ready', nudgeWv);
window.addEventListener('resize', nudgeWv);
if (window.api.onHomeShown) window.api.onHomeShown(nudgeWv);

// webview 로딩 상태 → 로딩 문구 숨김
wv.addEventListener('did-stop-loading', () => { loading.style.display = 'none'; nudgeWv(); });
