...
- **🎟🔨 복권·대장간 = 홈 창 딥링크(2026-07-08)**: 로직 복제 대신(둘 다 수시 밸런싱되는 시스템=복제하면 갈라짐) **상점 카테고리 「열기」 버튼 → homeWin(Shift+F6 홈 창)에서 홈페이지 함수 직접 호출**. IPC `open-home {goto}` → home.js `GOTO_JS`(lottery=`openLotteryHub()` / forge=`showTab('tab-shop')`+`gotoForgeTab()`)를 webview.executeJavaScript로 성공까지 재시도(0.5s×40). 데이터 100% 동일(홈페이지 그 자체). ⚠️webview partition=`persist:aram`이라 홈 창에서 **닉네임 최초 1회 선택 필요**(그 후 유지).
