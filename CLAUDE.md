...
- **수정**: ①main.js whenReady `config.isHost=false`(재실행마다 초기화)+`if(config.isHost)startLiveAccount()` 제거 → **자동 방장 안 함**(로그인서 방장 체크 시에만 setHost→startLiveAccount) ②sidepanel init `if(_myName)showLogged()` → **항상 `showLogin()`**+`_isHost=false`. 지난 아이디는 드롭다운에 `selected`로 미리 선택(편의)·방장 체크는 매번 꺼진 채. config.myName은 유지(오버레이 표시·드롭다운용). ⏭️ 비밀번호 로그인은 추후.
...
- **📑 사이드패널 헤더 재설계**: 방장 체크·계정변경으로 번잡+재화 잘림 → **헤더=재화 전용**(골드 크게 좌측 `sw-gold`+뽑기/투기장 코인 2줄 `sw-coins`) · **방장 체크→로그인 화면**(`.s-host-login`·입장 시 `setHost` 적용·`s-host` onchange는 `_isHost`만) · **계정변경→프로필 cat-hd 우측**(`.cat-hd-actions`·라이브 상태 점 `#s-live`도 여기로·`renderLiveStatus`가 텍스트→색점). `showLogin/showLogged` 정리(#s-me/#s-r2 제거·s-host checked는 showLogin서).
- **🛠️ 로비 "방장 준비 중" 배너**: 방장이 팀짜기서 참가자 고르는 중 → 홈페이지와 **동일 `/lobby` 노드**(`{state:'preparing',by,at,participants}`)로 팀원 대기화면에 "🛠️ N님이 팀 짜는 중 · ✅나를 선택함"(펄스 점). 홈 팀원도 자동(홈 기존 구독). main `fetchLobby`+pollSession 브로드캐스트+`lobby-prep/clear` IPC(방장 tbSync 디바운스 송신·탭이탈/시작/set-host off/quit 시 정리)·overlay `_lobbyBanner`·sidepanel `_lobbyPushDebounced`.
- **🥊 막고라(오버레이 생성 + 방장 결과입력)**: 팀짜기 탭 ⚔️일반/🥊막고라 토글(막고라=5명+·`_tbType`). 방장 시작→main `createMagollaMatch`(홈 openMagollaModal **파이터 LP가중 선정 이식**·`fbPush`·`bettingStartAt:Date.now()`=배팅 즉시 90초·`session/magollaMatchId` set→홈/오버레이 전파). **관전자 배팅=홈 임베드**(mg-bar「배팅/결과 열기」→hp-embed·홈 magolla모달 자동). **방장 결과입력=네이티브 폼**(`#mg-result`·배팅 마감[closed]후 승자/요건(tower/kill2/cs100)/증강(silver/gold/prisma) 3선택→`magolla-result` IPC→liveWin에 **`window.overlayMagollaResult` 주입**[홈 v2.45.576 신규·force-settle 패턴]→홈 `mgSettleMatch` 정산). 좌측 오버레이=파이터 대진 배너(레드 펄스). ⚠️mgGetMyRole은 liveMode만 'live'라 결과입력=라이브 전용 → liveWin 주입이 핵심. 폴백=`magolla-show-live`(liveWin 직접 표시·close→hide 가로채기).
- **🏠 홈 배포 의존**: `window.overlayMagollaResult`는 **홈 v2.45.576 배포됨**(라이브)이라야 오버레이 정산 주입 동작(liveWin=실사이트 로드). ⏭️ 실내전 검증(막고라 시작→배팅→마감→방장 3선택→정산 지급·로비 배너·새 헤더).
...
- **📐 해상도 적응성**(2026-07-10 확인·`applyDock`/`applyDockLeft` L101~123): 하드코딩 해상도 없음 — 클라 창 실측 rect를 실시간으로 읽어 **높이=클라 높이 정확 일치·Y=클라 상단·좌우 가장자리에 딱 붙음**. 모니터=`getDisplayMatching`(멀티모니터 OK·작업영역 기준=작업표시줄 제외)·DPI=`scaleFactor` 나눔(125/150% OK). **폭만 300~400px 고정 밴드**(클라 옆 남는 공간에 클램프·해상도 비례 아님·400↑=안 늘고 400에서 멈춤·300↓=클라 살짝 덮음). ⚠️알려진 한계 2: ①**혼합 DPI 멀티모니터**=`sf`를 `getPrimaryDisplay().scaleFactor`(주 모니터)에서 가져와 클라가 배율 다른 보조 모니터면 위치 어긋날 수 있음(같은 배율이면 무문제). ②고해상도서 폭 밴드가 상대적으로 작아 보일 수 있음(가독성 의도). 개선 원하면 ①클라 모니터의 scaleFactor 사용 ②폭을 남는공간 비례+상한.
...
- **🎟🔨 복권·대장간 = 오버레이 네이티브 재설계(v0.1.34·2026-07-08)** ★홈 임베드 폐기: 사장님 "임베드도 홈페이지 떠다 쓴 느낌 — 오버레이 톤으로 새로 만들되 데이터는 완전 연동". → v0.1.30 네이티브 렌더러 복원+전면 리디자인: **복권**=헥스텍 티켓 카드(절취선 스텁+티어 컬러 실버/골드/프리즘·해골/pity/걸작 배지·무료권), 긁기 모달=티어 프레임+당첨 글로우/파티클 burst+해골 쉐이크 / **대장간**=모루 스테이지(장착 강철심장 히어로 카드·등급색 오라)+**망치 스윙 모션+스파크(성공 골드/실패 레드)**+"모루에 올리기" / **SFX=WebAudio 합성(에셋 0)**: clang(노이즈+벨 배음)·ok/fail·jackpot·dud. **데이터 연동=구조적 보장**: 로직 전부 main store.js(홈 1:1·쓰기 직전 fetch-fresh·같은 Firebase 노드 pendingScratch_s2/lotteryHistory/emblem*)+조작잠금 B안 게이트 → 어느 쪽에서 하든 즉시 동일. 임베드 제거=sidepanel hpEmbedFor/워치독/HP_GOTO/#hp-embed 삭제(main.js webviewTag:true는 잔존·무해). 헤드리스 검증: 티켓3·모달 sct1/6칸·스테이지/버튼5·JS에러0. ⚠️**상시: 홈 복권/대장간/상점 밸런스 패치 시 store.js 상수(SCRATCH_TIERS·EMBLEM_TICKETS·EMBLEM_FX·pity) 미러링 필수**. ⚠️실기: 구매→긁기→정산·강화 모션/사운드 미확인.
- **⚡ 기록 로딩 가속+시즌2 고정+재화바 겹침 해결(v0.1.33·2026-07-08)**: 사장님 3건 — ①**기록 "불러오는 중" 무한**: 원인 2중 = (a) matches.json 수 MB 전체 다운로드 (b) **records-data 핸들러가 구버전(computeRecords)이어서 신 렌더러가 기대하는 shape(kind/detailA/players/bets)와 불일치→렌더 실패**(이전 세션 node 치환 무음 실패로 IPC 확장이 미적용 상태였음). 해결 = records-data가 {filter} 받아 computeRecordsEx 사용 + s2는 캐시 따뜻하면 캐시/아니면 `matches.json?orderBy="$key"&limitToLast=60`(fetchMatchesRecent) fast-path + magolla도 limitToLast=30 + 시작 2.5s 후 getMatchesCached() 예열 ②**랭킹 시즌2만**(시즌1 세그 제거·헤더에 SEASON 2 라벨·"나머지는 홈에서") — 기록 세그도 [내전/일반/막고라] 3개로 ③**상단 재화바 겹침**: 골드=골드 pill(14px)+코인=플레인 dim 텍스트(ellipsis·flex:1) ⚠️치환 무음 실패 교훈: node -e replace 후 반드시 grep 사후검증(이번에 records-data/ranking-data 미적용 2건 발견·수동 Edit로 복구).
- **🎨 임베드 다듬기+재화바(v0.1.32·2026-07-08)**: 사장님 "홈 통째로 보임=성의 없어 보임" → **홈 크롬 전부 제거**(insertCSS: header/.corner-badges-left/.live-mode-bar/#my-info-bar/.nav-tabs/footer/#attend-coach 숨김+.lh-close 숨김) = 복권 허브/대장간 "그 화면만" 패널 꽉 차게(네이티브룩). **복권 탭 워치독**(1.5s): 허브 닫히면 재오픈해 화면 고정 — 단 긁기(.scard-overlay)/구매확인/자동로봇(abot)/쓰레기통(.trash-overlay) 모달 중엔 개입 안 함. **상단 재화바 리디자인**: 골드=큰 골드 pill(15px)·코인=회색 pill·flex:1로 좌측 공간 채움(s-sp 숨김·로그인 화면선 복원).
- **📋 사장님 피드백 6+1건(v0.1.31·2026-07-08)**: ①팀짜기=「해제」(전체 선택해제) 버튼·LP 배지 제거(이름 온전히)·패딩 압축 ②상단바=아이디 대신 **재화 칩**(🪙골드·🕹️뽑기·🗡️투기장, wallet-data IPC·60초/탭전환 갱신·방장체크 유지) ③**복권·대장간 탭 = 홈페이지 임베드**(★네이티브 v0.1.29 구현 폐기→sidepanel 내 단일 `<webview partition=persist:aram>` 공유[#hp-embed 절대배치·탭 토글=재부착 리로드 방지]·딥링크 HP_GOTO[lottery=openLotteryHub/forge=showTab+gotoForgeTab, 0.5s×40 재시도]·insertCSS로 홈 네비/코너배지 숨김[상점 내부 소비/장비/대장간 카테고리는 유지=강화권 구매↔강화↔장착 한 화면 #6]·webviewTag:true leftWin 허용·뷰포트 고착 nudge) — **오른 3D·소리·유미 쓰레기통·자동로봇·긁개 도구 = 홈 100% 그대로**. store.js 복권/대장간 로직+IPC는 잔존(미사용·인게임 아이템뷰 emblem 표시는 계속 사용) ④기록 탭=필터 세그(시즌2/시즌1/일반/막고라, records-data {filter}·computeRecordsEx[custom=참가자 detailA/B+수상, normal=normal_matches, magolla=magolla_matches settled+베팅])+행 클릭 상세 펼침(.rc-det) ⑤랭킹 탭=TOP3 히어로 카드(.rk-hero 티어색 상단보더)+시즌 세그(2/1, ranking-data {season}·fetchLpSeason). ⚠️임베드 webview는 목업(브라우저)에선 미동작(호스트 토글만 검증)·실기 확인 필요.
