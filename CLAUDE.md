...
- **🎒 아이템 사용 뷰(오버레이·2026-07-06)**: 방장이 팀 짜기 시작하면 `session.phase='item'`(itemPhaseEnd=단계식별자·절대시각 아님·기기별 로컬 15초 카운트) → 오버레이에도 아이템 사용 창 자동 표시(방장 본인도 뜸). main.js pollSession `isItemPhase` 브랜치가 `fetchMyGold`+`fetchMyLpState` 병렬로 읽어 `itemphase` 이벤트 push. 오버레이는 전투 아이템 4종(🎲도박권 승+40/패−30·⚡LP2배권·🛡️승급전방어권·⚔️승급전승리권) 보유/활성 칩 표시·클릭 토글 → main.js `item-toggle` IPC가 `gold/{key}/items_s2` write. **상호배제(도박권↔LP2배권·방어권↔승리권)·조건(승급전권=promoActive만·도박/LP2배=placementDone&&!promoActive) 홈 toggleItemActive와 동일**. 뷰 우선순위 아이템>정산>투표>팀>명단(15초 뒤 팀 발표로 전환). 검증: 토글 상호배제 9/9·홈 원본 규칙 대조·헤드리스 렌더(4칩+타이머+활성상태) OK. ⚠️시너지 선택·강철심장 장착은 미이식(다음 단계)·실기 미검증.
